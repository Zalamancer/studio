...
import {
    ...
    AtSign, Briefcase, Info, Sparkles, User, Lightbulb, HelpingHand, FileQuestion, Brain, Target, Type, MessageCircle // Added MessageCircle for consistency
} from 'lucide-react';
...
import { generateAnonymousName, getInitials as getSharedInitials } from '@/lib/pseudonymUtils'; // Renamed import for clarity
...
const postFormSchema = z.object({
  ...
  imageFile: z.instanceof(File).optional().nullable()
    .refine(file => !file || file.size <= MAX_FILE_SIZE_BYTES, \`Max image size is ${MAX_FILE_SIZE_BYTES / 1024 / 1024}MB.\`)
    .refine(
  ...
  // Now, descriptionDetails is always the primary description.
  // No need for superRefine to check description vs descriptionDetails based on requestType.
  // Any specific validation for help_request fields can still be here if needed.
});

export interface CreatePostFormValues extends z.infer<typeof postFormSchema> {}

// This is the data structure the onSubmit prop will receive
export interface CreatePostFormData extends Omit<CreatePostFormValues, 'imageFile' | 'maxBudget' | 'tags' | 'deadline'> {
  imageFile?: File | null;
  mentionedUserIds?: string[];
  maxBudget?: number | null; // Ensure it can be null
  tags: string[];
  deadline?: Date | null; // Ensure it can be null
}
...
export const CreatePostForm: React.FC<CreatePostFormProps> = ({
  ...
  const [activeDescriptionTab, setActiveDescriptionTab] = useState("details"); // For controlling tabs
  ...
  useEffect(() => {
    if (onDialogClose && form.formState.isSubmitSuccessful) {
      const timer = setTimeout(() => {
        resetForm();
        if (onDialogClose) onDialogClose(); // Call the actual close handler
      }, 100);
  ...
  const evaluateMentionState = useCallback((
    ...
    if (lastAtIndex > -1 && (lastAtIndex === 0 || /\s|^$/.test(textBeforeCursor.charAt(lastAtIndex - 1)))) {
      ...
      if (!/\s/.test(potentialQuery) && !/\n|\r/.test(potentialQuery)) { // Corrected regex for newline
  ...
  const filteredDescriptionSuggestions = useMemo(() => {
    ...
    if (debouncedProblemDetailsQuery.trim() === '') {
        source = source.slice(0, 5); // Show first few if no query
    } else {
      ...
      <form onSubmit={form.handleSubmit(handleSubmitForm)} className="space-y-0">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6 p-1">
          ...
          <div className="space-y-6 flex flex-col">
                ...
                <FormItem className="space-y-3">
                  <FormLabel className="text-base font-semibold text-foreground">What kind of post is this? <span className="text-destructive">*</span></FormLabel>
                   <div className="flex items-center gap-3">
                     ...
                     >
                        <MessageCircle className={cn("mr-2 h-4 w-4", field.value === 'post' ? "text-primary-foreground" : "text-muted-foreground group-hover:text-foreground")}/> General Post
                     </Button>
                     <Button
                        type="button"
            ...
            <FormField control={form.control} name="imageFile" render={({ field }) => ( // field is not used directly, but needed for react-hook-form
              <FormItem>
        ...
        <DialogFooter className="pt-8 md:col-span-2">
            ...
            <Button type="submit" disabled={isSubmitting || isCompressing || (activeDescriptionTab === 'details' && showProblemDetailsSuggestions && isLoadingSuggestibleUsers)}>
                {(isSubmitting || isCompressing || (activeDescriptionTab === 'details' && showProblemDetailsSuggestions && isLoadingSuggestibleUsers) ) ? (<><Loader2 className="mr-2 h-4 w-4 animate-spin" /> {isCompressing ? "Processing..." : "Submitting..."}</>) : ('Submit Post')}
            </Button>
        </DialogFooter>
      </form>
       <AlertDialog open={showCompressionDialog} onOpenChange={setShowCompressionDialog}>
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                The selected image exceeds ${MAX_FILE_SIZE_BYTES / 1024 / 1024}MB ({(originalTooLargeFile?.size ? originalTooLargeFile.size / (1024 * 1024) : 0).toFixed(2)}MB).
...
// export default CreatePostForm; // Default export if this were its own file
```