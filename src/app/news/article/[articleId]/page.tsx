...
const ArticlePage = () => {
  ...
  const [storyContent, setStoryContent] = useState("<p><br></p>"); // Reverted initial content
  ...
  useEffect(() => {
    if (articleId) {
      ...
      setTitle("");
      setTags([]);
      setStoryContent("<p><br></p>"); // Reverted
  ...
  // Restore this useEffect with its original dependencies
  ...
  useEffect(() => {
    const handleInteraction = () => {
        if (contentEditableRef.current && (document.activeElement === contentEditableRef.current || (titleInputRef.current && document.activeElement === titleInputRef.current))) {
            // Wrap in requestAnimationFrame to allow browser to set caret first
    ...
    if (isEditingAllowed) {
        ...
        // Keep click on document if it was part of a working setup, but often direct editor click is better
        // For now, let's assume direct click handler on editor is sufficient.
        // If caret still jumps on click, document-level click with RAF might be needed.
    }
    return () => {
  ...
  const handleContentEditableInput = useCallback((event: React.SyntheticEvent<HTMLDivElement>) => {
    ...
    // Normalize empty content more simply
    const isEmptyContent = currentHTML.trim() === "" || currentHTML.trim() === "<br>" || currentHTML.trim() === "<p></p>";
    const finalHTML = isEmptyContent ? "<p><br></p>" : currentHTML;

    setStoryContent(finalHTML); // Unconditional update

    if (isEmptyContent && finalHTML === "<p><br></p>") { // Ensure DOM is updated if normalized
      if (event.currentTarget.innerHTML !== finalHTML) {
          event.currentTarget.innerHTML = finalHTML;
      }
      // Attempt to reposition caret after normalization
      queueMicrotask(() => {
  ...
  const handleContentKeyDown = useCallback((event: React.KeyboardEvent<HTMLDivElement>) => {
    ...
    // Sync state after other keydown events too
    setTimeout(() => {
  ...
  const validateFields = useCallback(() => {
    ...
    const currentEditorHTML = contentEditableRef.current?.innerHTML || "<p><br></p>"; // Reverted
  ...
  const handleUpdateArticle = async (
    ...
    const latestEditorHTML = contentEditableRef.current?.innerHTML || "<p><br></p>"; // Reverted
  ...
  const insertHTMLAndFocus = useCallback((htmlToInsert: string) => {
    ...
    queueMicrotask(() => {
      ...
      setSavedRange(null);
      const currentBlock = getCurrentBlockElement();
      if (currentBlock && editorEl.contains(currentBlock) && (currentBlock.textContent?.trim() === "" || currentBlock.innerHTML.toLowerCase() === "<br>" || currentBlock.innerHTML.toLowerCase() === "<p></p>" || currentBlock.innerHTML.toLowerCase() === "&nbsp;") && currentBlock.innerHTML.toLowerCase() !== "<p><br></p>") { // Reverted
        if (range.collapsed && (currentBlock.isSameNode(range.startContainer) || currentBlock.contains(range.startContainer))) {
          const isEditorAndEmptyOrSinglePBR = currentBlock.isSameNode(editorEl) && editorEl.innerHTML.trim().match(/^($|<br\s*\/?>|<p><br\s*\/?><\/p>|<p><\/p>)$/i); // Reverted
      ...
      setStoryContent(editorEl.innerHTML || "<p><br></p>"); // Reverted
  ...
  const handleEmbedDialogSubmit = () => {
    if (embedCodeInput) {
      ...
      if (sanitizedCode.trim()) insertHTMLAndFocus(`<div class="my-4 relative" data-embed-wrapper="true" contenteditable="false" style="padding-bottom: 100%; height: 0; overflow: hidden; max-width: 100%; border-radius: 0.25rem;"><div style="position: absolute; top: 0; left: 0; width: 100%; height: 100%;">${sanitizedCode}</div></div><p><br></p>`); // Reverted
  ...
  const handleInsertSeparator = useCallback(() => {
    ...
    insertHTMLAndFocus(`<hr class="my-8 border-border" /><p><br></p>`); // Reverted
  }, [insertHTMLAndFocus, isEditingAllowed]);
...
const CreateNewsArticlePage = () => {
  ...
  const [title, setTitle] = useState("");
  const [tags, setTags] = useState<string[]>([]);
  const [storyContent, setStoryContent] = useState("<p><br></p>"); // Reverted
  const [coverImageFile, setCoverImageFile] = useState<File | null>(null);
  const [coverImagePreview, setCoverImagePreview] = useState<string | null>(null);
  ...
  const [isHeaderSearchActive, setIsHeaderSearchActive] = useState(false); // Retained from previous merge
  const [headerSearchTerm, setHeaderSearchTerm] = useState(''); // Retained from previous merge
  ...
  // Restore document-level click listener
  useEffect(() => {
    const handleInteraction = () => {
    ...
    document.addEventListener('selectionchange', handleInteraction);
    document.addEventListener('keyup', handleInteraction);
    document.addEventListener('click', handleInteraction); // Restored
    return () => {
      document.removeEventListener('selectionchange', handleInteraction);
      document.removeEventListener('keyup', handleInteraction);
      document.removeEventListener('click', handleInteraction); // Restored
    };
  ...
  const handleContentEditableInput = useCallback((event: React.SyntheticEvent<HTMLDivElement>) => {
    const currentHTML = event.currentTarget.innerHTML;
    const isEmptyContent = currentHTML.trim() === "" || currentHTML.trim() === "<br>" || currentHTML.trim() === "<p></p>"; // Reverted
    const finalHTML = isEmptyContent ? "<p><br></p>" : currentHTML; // Reverted
    
    setStoryContent(finalHTML); // Unconditional setStoryContent

    if (isEmptyContent && finalHTML === "<p><br></p>") { // Reverted
      // Ensure DOM matches if normalized
      if (event.currentTarget.innerHTML !== finalHTML) {
          event.currentTarget.innerHTML = finalHTML;
      }
      // Attempt to reposition caret after normalization
      queueMicrotask(() => {
        if (contentEditableRef.current) {
          const pTag = contentEditableRef.current.querySelector('p'); // Simplified query
          if (pTag) {
            const range = document.createRange();
            const sel = window.getSelection();
            try {
  ...
  const handleContentKeyDown = useCallback((event: React.KeyboardEvent<HTMLDivElement>) => {
    ...
    const selection = window.getSelection(); if (!selection || selection.rangeCount === 0) return;
    const range = selection.getRangeAt(0); const currentBlock = getCurrentBlockElement();
    if (event.key === 'Enter') { event.preventDefault(); document.execCommand('insertParagraph', false, undefined); if (editorEl) setStoryContent(editorEl.innerHTML); setTimeout(() => { if (contentEditableRef.current) { updateSelectionNonce();}}, 0); return; } // Sync state
    if (event.key === 'Backspace' || event.key === 'Delete') {
      if (range.collapsed && currentBlock) {
        const focusNode = selection.focusNode; const focusOffset = selection.focusOffset; let isAtBoundary = false;
        if (event.key === 'Backspace') {
          ...
          const prevElement = currentBlock.previousElementSibling;
          if (isAtBoundary && prevElement && (prevElement.tagName === 'FIGURE' || prevElement.getAttribute('data-embed-wrapper') === 'true' || prevElement.tagName === 'PRE' || prevElement.tagName === 'HR')) {
            event.preventDefault(); prevElement.remove(); if (editorEl) setStoryContent(editorEl.innerHTML); updateSelectionNonce(); return; // Sync state
          }
        } else { 
          if ((focusNode === currentBlock && focusOffset === currentBlock.childNodes.length) || (focusNode && focusNode.nodeType === Node.TEXT_NODE && currentBlock.contains(focusNode) && focusOffset === focusNode.textContent?.length && !focusNode.nextSibling) || (focusNode && focusNode.nodeType === Node.ELEMENT_NODE && currentBlock.lastChild === focusNode && focusOffset === focusNode.childNodes.length && (focusNode.textContent === "" || (focusNode as HTMLElement).tagName === 'BR'))) isAtBoundary = true;
          const nextElement = currentBlock.nextElementSibling;
          if (isAtBoundary && nextElement && (nextElement.tagName === 'FIGURE' || nextElement.getAttribute('data-embed-wrapper') === 'true' || nextElement.tagName === 'PRE' || nextElement.tagName === 'HR')) {
            event.preventDefault(); nextElement.remove(); if (editorEl) setStoryContent(editorEl.innerHTML); updateSelectionNonce(); return; // Sync state
          }
        }
      }
    }
    setTimeout(() => { if (editorEl) setStoryContent(editorEl.innerHTML); updateSelectionNonce(); },0); // Sync state
  ...
  const handleFormSubmission = async (status: NewsArticleStatus) => {
    ...
    const finalStoryContent = contentEditableRef.current?.innerHTML || "<p><br></p>"; // Reverted, read from ref
    try {
      if (coverImageFile) {
      ...
      const articleData: NewNewsArticleData = {
        userId: user.uid, title: title.trim(), tags: tags, content: finalStoryContent, status, coverImageUrl,
      };
      ...
      setTitle(""); setTags([]); setStoryContent("<p><br></p>"); setCoverImageFile(null); setCoverImagePreview(null); // Reverted
  ...
  const insertHTMLAndFocus = useCallback((htmlToInsert: string) => {
    ...
    queueMicrotask(() => {
      ...
      if (selection) { selection.removeAllRanges(); selection.addRange(range); }
      setStoryContent(editorEl.innerHTML || "<p><br></p>"); setIsToolbarExpanded(false); // Reverted
      queueMicrotask(() => { editorEl.focus(); updateSelectionNonce(); });
    });
  ...
  const toggleHeaderSearch = () => { // Retained from previous merge
  ...
  return (
    <>
    <div className="container mx-auto py-8 px-4 md:px-6">
      {/* Header with toggleable search - Retained and adapted from a previous merge */}
      <div className="flex items-center justify-between mb-4 gap-2 flex-wrap">
        <Button variant="outline" size="sm" onClick={() => router.push('/news')} className="text-xs h-9 px-3 flex-shrink-0">
          <ArrowLeft className="mr-2 h-4 w-4" /> Back to News
        </Button>
      ...
      <div ref={formWrapperRef} className="max-w-3xl mx-auto relative pt-5">
        ...
        <div ref={titleWrapperRef} className="relative mb-4">
          {/* Restored direct onClick and onKeyUp for title input */}
          <Input ref={titleInputRef} placeholder="Title" value={title} onChange={(e) => { setTitle(e.target.value); if (publishAttempted) { if (e.target.value.trim()) setTitleError(""); else setTitleError("Title is required."); } updateSelectionNonce(); }} onFocus={() => handleFocus('title')} onBlur={handleBlur} onKeyUp={updateSelectionNonce} onClick={updateSelectionNonce} className="text-4xl lg:text-5xl font-bold border-0 focus-visible:ring-0 focus-visible:ring-offset-0 shadow-none px-0 placeholder:text-muted-foreground/50 h-auto py-2" autoComplete="off" disabled={isSubmitting} />
          {publishAttempted && titleError && <p className="text-xs text-destructive mt-1">{titleError}</p>}
        </div>
        <div ref={contentWrapperRef} className="relative">
          <div
            ...
            onClick={updateSelectionNonce} // Restored
            onKeyUp={updateSelectionNonce}  // Restored
            data-placeholder="Tell your story..."
            className={cn("w-full rounded-md border-0 focus-visible:ring-0 focus-visible:ring-offset-0 shadow-none px-0 placeholder:text-muted-foreground/50 py-2 font-normal text-start no-underline tracking-normal whitespace-pre-wrap break-words normal-case", "focus:outline-none min-h-[150px]")}
            style={{ fontFamily: "medium-content-sans-serif-font, -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Oxygen, Ubuntu, Cantarell, 'Open Sans', 'Helvetica Neue', sans-serif", fontSize: "20px", lineHeight: "1.6", color: "hsl(var(--foreground))", direction: 'ltr' }} // Kept `dir="ltr"` on parent
            role="textbox" aria-multiline="true" aria-label="News article content" suppressContentEditableWarning={true} dir="ltr"
            dangerouslySetInnerHTML={{ __html: storyContent }}
          />
        </div>
