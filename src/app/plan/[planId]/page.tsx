...
import {
  ...
  Unlink,
  PlusCircle,
  ListTree, // Corrected: Using ListTree as a placeholder for child indicator
} from 'lucide-react';
import {
  AlertDialog,
  AlertDialogAction,
...
const CHILD_NODE_ITEM_HEIGHT = 24; // Approx height per child item in list
...
const calculateNodeHeight = (step: RoadmapStep, allSteps: RoadmapStep[]): number => {
  ...
  if (step.description && step.description.trim().length > 0) {
    // Rough line calculation: average 35 chars per line in the constrained width.
    // Add 1 for each explicit newline.
    const lines = Math.ceil(step.description.length / 35) + (step.description.split(/\r\n|\r|\n/).length - 1);
    descriptionLineCount = Math.max(1, lines);
  }
  const descriptionHeight = descriptionLineCount * 15; // Approx 15px per line height

  let childNodesListHeight = 0;
  const childCanvasNodes = allSteps.filter(s => s.parentId === step.id);
  if (childCanvasNodes.length > 0) {
    childNodesListHeight = (childCanvasNodes.length * CHILD_NODE_ITEM_HEIGHT) + (CHILD_NODE_ITEM_HEIGHT / 2); // Add some buffer
  }

  const contentHeight = Math.max(descriptionHeight, childNodesListHeight);
  height += contentHeight;
  height += FINAL_BUFFER_CARD_HEIGHT; // Final buffer
  ...
  return (
    <div
      ...
      style={{
        ...
        touchAction: 'none',
        overflow: 'visible', // Dots are positioned outside
        backgroundColor: 'hsl(var(--card))', // Ensure card bg for consistency
      }}
      onMouseDown={(e) => onNodeInteractionStart(step.id, e)}
      onTouchStart={(e) => onNodeInteractionStart(step.id, e)}
      onClick={(e) => {
        if (isActuallyDraggingThisNode) { // Prevent click when drag finishes
            e.stopPropagation();
            return;
        }
        // If it was a true click (not a drag end), select and open for edit
        if (!isSelected) { // Only open if not already selected to avoid closing/reopening on drag end.
           onEditStep(step);
        }
      }}
      data-node-id={step.id} // For querying the DOM element if needed
    >
      <div
        className="p-2 border-b border-border flex items-center justify-between cursor-move rounded-t-lg h-[40px]" // Fixed header height
        style={{ backgroundColor: 'hsl(var(--primary))' }} // Use primary color for header
        onDoubleClick={() => onEditStep(step)} // Open edit panel on header double click
      >
        <h3 className="text-sm font-semibold truncate text-primary-foreground" title={step.title}>{step.title}</h3>
        {/* Actions like Edit/Delete for the card can go here, consider a small dropdown */}
      </div>
      <div className="p-2 text-xs flex-grow min-h-0 space-y-1" style={{ backgroundColor: 'hsl(var(--card))' }}
           onDoubleClick={() => onEditStep(step)} // Open edit panel on content double click
      >
...
interface LineContextMenuState {
  ...
  x: number;
  y: number;
  childNodeId: string; // The child node whose parent link is being clicked
}
...
export default function PlanDetailPage() {
  ...
  const [canvasMinHeight, setCanvasMinHeight] = useState<number>(typeof window !== 'undefined' ? window.innerHeight : 800); // Default height
  ...
  // State for managing the currently selected/editing step in the panel
  ...
  // State for node deletion confirmation
  ...
  const isDraggingRef = useRef<boolean>(false); // To distinguish click from drag
  ...
  interface ConnectionDragInfo {
    type: 'connectionDot';
    sourceNodeId: string;
    sourceAnchor: 'N' | 'S' | 'E'; // Anchor point on the source node
    startX: number; // Canvas coordinates of the dot click
    startY: number;
  }
  ...
  const { data: planData, isLoading: isLoadingPlan, error: planError } = useQuery<ClientPlan | null>({
    ...
    enabled: !!planId && isValidPlanId && !authLoading, // Fetch only if planId is valid and auth is not loading
  });
  ...
  const { data: planVersions = [], isLoading: isLoadingVersions, error: versionsError } = useQuery<ClientPlanVersion[]>({
    ...
    enabled: isVersionHistorySheetOpen && !!planId && isValidPlanId, // Fetch only when sheet is open
  });
  ...
  useEffect(() => {
    // Dynamically adjust canvas min-height based on content
    if (typeof window !== 'undefined') {
      ...
      // Ensure canvas is at least viewport height, or tall enough for content + some scroll room
  ...
  const handleEditStep = useCallback((stepToEdit: RoadmapStep) => {
    ...
    setIsEditingNodeTitle(false); // Reset editing states when opening
  ...
  const handleAddNode = useCallback((data: AddRoadmapStepFormData) => {
    ...
    const newNode: RoadmapStep = {
      ...
      x: newStepX,
      y: newStepY,
      parentId: targetParentIdForDialog, // This will be null for root nodes
    };
    ...
    if (targetParentIdForDialog) { // Creating a child node
        ...
        } else { // Fallback if parent not found (should ideally not happen)
    ...
    } else if (canvasRef.current) { // Creating a root node (no parent target)
      // Position new root node in the center of the current view
      newNode.x = Math.max(MIN_CANVAS_PADDING, canvasRef.current.scrollLeft + canvasRef.current.clientWidth / 2 - NODE_BASE_WIDTH / 2);
      newNode.y = Math.max(MIN_CANVAS_PADDING, canvasRef.current.scrollTop + canvasRef.current.clientHeight / 2 - NODE_BASE_MIN_HEIGHT / 2);
    }
  ...
  const handleInitiateAddChildToNode = useCallback((parentId: string | null) => {
    if (!canEditPlan) return;
    setTargetParentIdForDialog(parentId); // Set the target parent for the new node
  ...
  const confirmDeleteNode = useCallback(() => {
    ...
    setEditableRoadmap(prev => {
        ...
        const updatedChildren = prev
            ...
            .map(child => ({ ...child, parentId: null, x: child.x + 10, y: child.y + 10 })); // Slight offset to make them visible
    ...
    if (editingStep?.id === idToDelete) {
        setIsStepDetailSheetOpen(false); // Close sheet if deleted node was being edited
        setEditingStep(null);
    }
  ...
  const handleStepDetailUpdate = useCallback((updatedStep: RoadmapStep) => {
    ...
    setIsEditingNodeTitle(false); setIsEditingNodeDescription(false); // Exit editing modes
  }, [canEditPlan, toast]);

  const saveRoadmapChanges = useCallback(async () => {
    if (!planData || !user || !planId || !canEditPlan) {
      toast({ variant: "destructive", title: "Error", description: "Cannot save: Plan data, user authentication, or permissions missing." });
      return;
    }
  ...
  const sharePlan = useCallback(async () => {
    try { await navigator.clipboard.writeText(window.location.href); toast({ title: "Link Copied!", description: "Plan URL copied to clipboard." }); }
    catch (err) { toast({ variant: "destructive", title: "Copy Failed", description: "Could not copy link to clipboard." }); }
  }, [toast]);

  const handleLineClick = useCallback((event: React.MouseEvent<SVGPathElement>, childNodeId: string) => {
    if (!canEditPlan) return;
    event.preventDefault(); event.stopPropagation(); // Important
    const canvasRect = canvasRef.current?.getBoundingClientRect();
  ...
  const handleDetachParent = useCallback((childNodeId: string) => {
    ...
    toast({ title: "Node Detached", description: "Node is now a root node. Remember to save the plan." });
    setLineContextMenu(null); // Close context menu
  }, [canEditPlan, toast]);
  ...
  const handleNodeInteractionStart = useCallback((nodeId: string, event: React.MouseEvent<HTMLDivElement> | React.TouchEvent<HTMLDivElement>) => {
    ...
    if (lineContextMenu?.isOpen) setLineContextMenu(null); // Close context menu on new interaction
    ...
    connectionDragInfoRef.current = null; // Clear connection drag info
  ...
  const handleDotInteractionStart = useCallback((nodeId: string, clickedAnchor: 'N' | 'S' | 'E', event: React.MouseEvent<HTMLElement> | React.TouchEvent<HTMLElement>) => {
    ...
    const dotCenterCanvas = getElementCenter(dotElement); // Get dot center relative to canvas
  ...
  const isCyclical = (potentialChildId: string, potentialParentId: string, currentRoadmap: RoadmapStep[]): boolean => {
    ...
    const visited = new Set<string>(); // To detect loops in malformed data, though parentId should prevent direct graph cycles.
    while(current && !visited.has(current)) {
        visited.add(current);
        if (current === potentialChildId) return true; // Found the child in its own ancestry path
        const node = currentRoadmap.find(n => n.id === current);
        current = node?.parentId || ""; // Move to the parent of the current node
    }
  ...
  const handleGlobalMove = useCallback((event: MouseEvent | TouchEvent) => {
    ...
    if (clickStartInfoRef.current && !isDraggingRef.current) {
      ...
      if ((deltaX * deltaX + deltaY * deltaY) > CLICK_MOVE_THRESHOLD_PX_SQ) { // If moved more than 5px
        isDraggingRef.current = true;
      }
    }

    if (isDraggingRef.current) {
      if (event.cancelable) event.preventDefault(); // Prevent page scroll on touch devices during drag
      const canvasRect = canvasRef.current.getBoundingClientRect();
      ...
      if (nodeDragInfoRef.current) {
        ...
        newX = Math.max(MIN_CANVAS_PADDING, newX); // Clamp to canvas bounds
  ...
  const handleGlobalPointerUp = useCallback((event: MouseEvent | TouchEvent) => {
    ...
    if (nodeDragInfoRef.current && !isDraggingRef.current && clickInfo) {
        ...
        // It was a drag from a connection dot
        ...
        for (const targetStep of editableRoadmap) {
            if (targetStep.id === sourceNodeId) continue; // Can't connect to self
            const targetNodeHeight = calculateNodeHeight(targetStep, editableRoadmap);
            const targetRect = {
                left: targetStep.x,
                top: targetStep.y,
                right: targetStep.x + NODE_BASE_WIDTH,
                bottom: targetStep.y + targetNodeHeight
            };
            if (releaseX >= targetRect.left && releaseX <= targetRect.right &&
                releaseY >= targetRect.top && releaseY <= targetRect.bottom) {
        ...
        if (targetNodeFound) { // Connecting to an existing node
            ...
            if (sourceAnchor === 'S') { // Dragging from bottom of source to make source child of target
                childToSetParentId = sourceNodeId;
                newParentIdForChild = targetNodeFound.id;
            } else if (sourceAnchor === 'N') { // Dragging from top of source to make target child of source
                childToSetParentId = targetNodeFound.id;
                newParentIdForChild = sourceNodeId;
            } else if (sourceAnchor === 'E') { // Dragging from side of source to make source child of target
                childToSetParentId = sourceNodeId;
                newParentIdForChild = targetNodeFound.id;
            }
            // West dot is removed, so no 'W' case
            ...
            if (childToSetParentId && newParentIdForChild && childToSetParentId !== newParentIdForChild) {
                if (!isCyclical(childToSetParentId, newParentIdForChild, editableRoadmap)) {
                    setEditableRoadmap(prev => prev.map(s =>
                        s.id === childToSetParentId ? { ...s, parentId: newParentIdForChild } : s
                    ));
            ...
            setTargetParentIdForDialog(sourceNodeId); // Set source node as parent
            setIsAddNodeDialogOpen(true); // Open dialog to name new child
        }
    }

    // Reset all drag/click states
    nodeDragInfoRef.current = null;
  ...
  useEffect(() => {
    if (isPointerDown) {
      ...
      window.addEventListener('touchmove', handleGlobalMove, { passive: false }); // passive: false to allow preventDefault
      window.addEventListener('mouseup', handleGlobalPointerUp);
      window.addEventListener('touchend', handleGlobalPointerUp);
      window.addEventListener('touchcancel', handleGlobalPointerUp); // Handle touch cancel as well
    }
    return () => {
  ...
  const drawConnectionLines = () => {
    return editableRoadmap
      .filter(childStep => childStep.parentId) // Only draw lines for nodes that have a parent
      .map(childStep => {
        ...
        // South dot of parent: center-bottom of the node card
        const parentAnchorPoint = {
          x: parentStep.x + NODE_BASE_WIDTH / 2,
          y: parentStep.y + parentNodeHeight // This is the bottom edge of the card
        };
        // North dot of child: center-top of the node card
        const childAnchorPoint = {
          x: childStep.x + NODE_BASE_WIDTH / 2,
          y: childStep.y // This is the top edge of the card
        };
  ...
  // For selecting a child node from the parent's card list or sheet list
  const handleSelectChildNodeOnCanvas = useCallback((childNodeId: string) => {
    ...
    if (childNode) {
      handleEditStep(childNode); // Open the child node in the edit panel
      // Scroll the canvas to bring the child node into view
  ...
  // Conditional Rendering for loading/error/no data states
  ...
  // Get children for the currently editing step (for the panel)
  const directChildrenOfEditingStep = editingStep ? editableRoadmap.filter(s => s.parentId === editingStep.id) : [];

  return (
    <div className="flex flex-col flex-1 w-full overflow-hidden"> {/* Ensure full height and width */}
      {/* Header Bar */}
      ...
      {/* Main Canvas Area */}
      <div className="flex flex-1 overflow-hidden"> {/* This div will contain the canvas */}
            ...
            <PopoverContent className="w-auto p-1" side="right" align="start" sideOffset={5}>
              {lineContextMenu && canEditPlan && (
                <div className="flex flex-col gap-1">
                  ...
                  {/* Add more actions here if needed, e.g., "Change Line Style" */}
                </div>
              )}
            </PopoverContent>
          </Popover>
      ...
      {/* Sheet for Editing Node Details */}
            ...
            <>
              ...
              <ScrollArea className="flex-grow min-h-0">
                <div className="p-4 space-y-4">
                    ...
                    {/* Child Nodes List (on canvas) */}
                    <div className="space-y-2 mt-3">
                        <Label className="flex items-center"><ListTree className="mr-1.5 h-4 w-4 text-primary/80"/>Child Nodes (On Canvas)</Label>
                    ...
                    {/* The old "Sub-steps" (tasks) section has been removed. */}
                </div>
              </ScrollArea>
      ...
      {/* Dialog for Confirming Node Deletion */}
      ...
      {/* Sheet for Version History */}
      <Sheet open={isVersionHistorySheetOpen} onOpenChange={setIsVersionHistorySheetOpen}>
        <SheetContent className="sm:max-w-lg w-[90vw]" side="left"> {/* Wider for better version display */}
          ...
          <ScrollArea className="h-[calc(100%-100px)]"> {/* Adjust height to fit footer */}
